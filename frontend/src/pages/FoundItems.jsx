...
const FoundItemCard = ({ item, onViewDetails }) => (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden flex flex-col">
        <div className="w-full h-48 bg-gray-100 overflow-hidden">
            <img src={item.photoUrl || 'https://placehold.co/600x400/E2E8F0/A0AEC0?text=No+Image'} alt={item.item} className="w-full h-full object-cover" />
        </div>
        <div className="p-5 flex flex-col flex-grow">
            <h3 className="text-lg font-bold text-gray-500 uppercase tracking-wider">{item.item}</h3>
            <p className="text-base text-gray-700 font-semibold mt-2">Found by: {item.name}</p>
            <p className="text-sm text-gray-500 mt-1">Location: {item.location}</p>
            <p className="text-sm text-gray-500 mt-1">Date: {formatDate(item.date)}</p>
...
const FoundItemsPage = () => {
    ...
    // --- UPDATED: Fetching data from your backend ---
    useEffect(() => {
        const fetchFoundItems = async () => {
            try {
                // This makes a request to your backend's GET /api/found endpoint
        ...
        <div className="bg-gray-100 min-h-screen font-sans">
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
                    ...
                    <Link to="/report-found" className="w-full md:w-auto flex items-center justify-center bg-green-500 text-white font-semibold py-3 px-6 rounded-lg shadow-md hover:bg-green-600 transition-colors duration-300">
                        <PlusIcon />
                        Report an Item
                    </Link>
                </div>
                ...
                {loading && <div className="text-center py-16 text-gray-500">Loading items...</div>}
                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                            {filteredItems.map(item => (
                                <FoundItemCard key={item._id} item={item} onViewDetails={handleViewDetails} />
                            ))}
                        </div>
                        {filteredItems.length === 0 && (
                            <div className="text-center py-16">
                                <h3 className="text-xl font-semibold text-gray-700">No Items Found</h3>
                                <p className="text-gray-500 mt-2">Your database might be empty, or there was a problem fetching the data.</p>
                            </div>
                        )}
                    </>
                )}
