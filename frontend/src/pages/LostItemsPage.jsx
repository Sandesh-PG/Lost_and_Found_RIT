                ...
                <div className="flex flex-col md:flex-row gap-8">
                    ...
                    <div className="md:w-1/2 flex flex-col">
                        ...
                        <div className="mt-4 space-y-3 text-gray-600">
                            <p><strong>Lost by:</strong> {item.name}</p>
...
const LostItemCard = ({ item, onViewDetails }) => (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden flex flex-col">
        ...
        <div className="p-5 flex flex-col flex-grow">
            ...
            <p className="text-base text-gray-700 font-semibold mt-2">Lost by: {item.name}</p>
        ...
        <div className="bg-gray-100 min-h-screen font-sans">
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
                    ...
                    <Link to="/report-lost" className="w-full md:w-auto flex items-center justify-center bg-red-500 text-white font-semibold py-3 px-6 rounded-lg shadow-md hover:bg-red-600 transition-colors duration-300">
                        <PlusIcon />
                        Report a Lost Item
                    </Link>
                </div>
                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                            {filteredItems.map(item => (
                                <LostItemCard key={item._id} item={item} onViewDetails={handleViewDetails} />
                            ))}
                        </div>
                        {filteredItems.length === 0 && (
                            <div className="text-center py-16">
                                <h3 className="text-xl font-semibold text-gray-700">No Items Found</h3>
                                <p className="text-gray-500 mt-2">No one has reported a lost item yet. Be the first!</p>
                            </div>
                        )}
                    </>
                )}
